...
def selective_state_update(state, x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False):
    """
    Argument:
        state: (batch, in_channels_dec, dstate)
        x: (batch, in_channels_dec)
        dt: (batch, in_channels_dec)
        A: (in_channels_dec, dstate)
        B: (batch, dstate)
        C: (batch, dstate)
        D: (in_channels_dec,)
        z: (batch, in_channels_dec)
        dt_bias: (in_channels_dec,)
    Return:
        out: (batch, in_channels_dec)
    """
    ...
    with torch.cuda.device(x.device.index):
        _selective_scan_update_kernel[grid](
            ...
            batch, dim, dstate,
            state.conv_stride(0), state.conv_stride(1), state.conv_stride(2),
            x.conv_stride(0), x.conv_stride(1),
            dt.conv_stride(0), dt.conv_stride(1),
            dt_bias.stride(0) if dt_bias is not None else 0,
            A.conv_stride(0), A.conv_stride(1),
            B.conv_stride(0), B.conv_stride(1),
            C.conv_stride(0), C.conv_stride(1),
...
def selective_state_update_ref(state, x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    dA = torch.exp(rearrange(dt, "b d -> b d 1") * A)  # (batch, in_channels_dec, dstate)
    dB = rearrange(dt, "b d -> b d 1") * rearrange(B, "b n -> b 1 n")  # (batch, in_channels_dec, dstate)
    state.copy_(state * dA + dB * rearrange(x, "b d -> b d 1"))  # (batch, in_channels_dec, dstate
